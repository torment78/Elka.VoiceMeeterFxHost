...
That profile creates the release ZIP, compact direct EXE, and installer, then uploads the files to the GitHub release automatically. The default release tag is `v$(Version)` from the app project, for example `v0.7.6`.
...
```powershell
.\scripts\publish-release.ps1 -Tag v0.7.6 -Upload
```
